...
- **Status (2026-07-01): IMPLEMENTED** — branch `phase1-refresh-rotation`, plan `docs/superpowers/plans/2026-07-01-refresh-token-rotation.md`. Behavior as specified: `auth_sessions` now stores one row per issued refresh token, each carrying `family_id VARCHAR(64) NOT NULL` (login lineage), `status VARCHAR(16) NOT NULL` (`ACTIVE`/`ROTATED`/`REVOKED`), and `rotated_at TIMESTAMPTZ` (Flyway V2; existing rows backfilled to single-token `ACTIVE` families so live sessions survive the deploy). **Rotate path:** presenting an `ACTIVE`, unexpired, user-matched token retires it (`ROTATED` + `rotated_at=now`) and inserts a fresh `ACTIVE` row in the same family; new tokens returned. **Reuse path:** presenting a token with `status != ACTIVE` bulk-revokes the entire family (`UPDATE auth_sessions SET status='REVOKED' WHERE family_id=?`), writes a `REFRESH_TOKEN_REUSE_DETECTED` audit event to `identity.rbac_audit_log` (carrying `user_id`, `correlation_id` = family_id, `details`), and returns 401 — the revoke + audit commit is guaranteed even on 401 via `@Transactional(noRollbackFor=ResponseStatusException.class)`. Unknown / expired token hash → generic 401, no family touched. **Logout** revokes the whole family. **Blast radius is family-only** (never all families of the user). Access TTL unchanged (~15 min / 900 000 ms); refresh TTL unchanged (7 days). Test coverage: `AuthSessionMigrationTest` (2 — Testcontainers, proves V2 migration + backfill); `IdentityAuthServiceRotationTest` (7 — Mockito, covers rotate/reuse/family-scoped-blast-radius/logout/unknown-token); `IdentityAuthServiceReuseCommitIntegrationTest` (1 — Testcontainers, proves audit row persists across 401 rollback boundary).
...
- **Status (2026-07-01):** Validated-DTO pattern + `{message, fieldErrors}` 400 error contract piloted in **identity-service** (`phase1-input-validation`). RBAC write endpoints (createRole, updateRole, assign platform/school/zone role) + password-reset converted to `@Valid` DTO records; per-service `ValidationExceptionHandler` (`@RestControllerAdvice`) handles `MethodArgumentNotValidException` and `ConstraintViolationException`. Boundary-only (repos unchanged). Remaining identity endpoints + the other ~11 services to replicate the pattern — **[EXPAND per service]**.

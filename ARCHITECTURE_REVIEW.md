...
- **RLS vs the single-`appuser` decision:** the prior plan collapsed to one owner role; **RLS needs an unprivileged runtime role** — reconcile these two before implementing (see MT-P0-2).
  ...
  - **Phase 1 BOLA gate — MT CI gate / SEC-P3-1 partial (2026-07-01):** an automated BOLA regression suite (`scripts/audit-tenant-isolation.ps1`) now runs in `whole-application-validation` CI as a required gate. The suite provisions a two-school seeded fixture (`ensure-app-rt-local.ps1` + `ensure-local-dev-users.ps1`), establishes a positive baseline (tenant-A admin can read tenant-A data), then executes **13 cross-tenant probes** — detail-by-id and list-with-school-id-param across students, catalog orders, firefighting requests, and workflow instances — asserting 403/404/empty for every tenant-A token / tenant-B resource pair. Gateway runs in **enforce mode** (`docker-compose.bola.yml` overlay) for the gate so that token-injected `school_id` is authoritative. Wired into the verifier via `-RunBolaAudit` switch (`verify-microservice-migration.ps1`). **All 13 probes isolated — no cross-tenant leaks found.**

terraform {
  ...
  #   terraform init -backend-config="bucket=custoking-prod-terraform-state" -backend-config="prefix=cicd"
  # NOT custoking-terraform-state -- that bucket belonged to the pre-split project and is gone.

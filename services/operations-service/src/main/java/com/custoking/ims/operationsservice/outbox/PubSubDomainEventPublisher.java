 ...
 *
 * <p>Only active when {@code operations.outbox.pubsub.topic-id} (env
 * {@code OPERATIONS_OUTBOX_PUBSUB_TOPIC_ID}) is configured — i.e. in deployed
 ...
 */
@Component
@ConditionalOnProperty(name = "operations.outbox.pubsub.topic-id")
public class PubSubDomainEventPublisher implements DomainEventPublisher {
    ...
    public PubSubDomainEventPublisher(
            ...
            @Value("${operations.outbox.pubsub.topic-id}") String topicId,
            @Value("${operations.outbox.pubsub.project-id:}") String projectId) throws IOException {
    ...
    private static String resolveProjectId(String configuredProjectId) {
        ...
        // Prod sets operations.outbox.pubsub.project-id (OPERATIONS_OUTBOX_PUBSUB_PROJECT_ID=$PROJECT_ID
        // in cloudbuild), so this throw only fires on a misconfiguration.
        throw new IllegalStateException(
                "operations.outbox.pubsub.topic-id is set but no project id is configured "
                        + "(operations.outbox.pubsub.project-id / GOOGLE_CLOUD_PROJECT / GCP_PROJECT)");
    }

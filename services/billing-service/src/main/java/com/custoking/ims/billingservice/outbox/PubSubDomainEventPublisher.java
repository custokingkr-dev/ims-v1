 ...
 *
 * <p>Only active when {@code billing.outbox.pubsub.topic-id} (env
 * {@code BILLING_OUTBOX_PUBSUB_TOPIC}) is configured — i.e. in deployed
 ...
 */
@Component
@ConditionalOnProperty(name = "billing.outbox.pubsub.topic-id")
public class PubSubDomainEventPublisher implements DomainEventPublisher {
    ...
    public PubSubDomainEventPublisher(
            ...
            @Value("${billing.outbox.pubsub.topic-id}") String topicId,
            @Value("${billing.outbox.pubsub.project-id:}") String projectId) throws IOException {

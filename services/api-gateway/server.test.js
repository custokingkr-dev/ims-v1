...
test('student photo import, guardian review, and assigned-school export routes go to student service', () => {
  const resolve = (p) => routes.find((r) => r.matches(p))?.service;
  ...
  assert.equal(resolve('/api/v1/guardian-data-review/summary'), 'student');
  assert.equal(resolve('/api/v1/guardian-data-review/cases/abc/decisions'), 'student');
});

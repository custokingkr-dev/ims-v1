...
public class BroadcastDispatchRepository {
    ...
    public Broadcast find(UUID id, boolean lock) {
        return jdbc.sql("SELECT id, school_id, title, message, audience_type, channels, communication_category, status, dispatch_mode FROM notification.notification_broadcasts WHERE id = :id" + (lock ? " FOR UPDATE" : ""))
                .param("id", id).query((rs, row) -> new Broadcast(rs.getObject("id", UUID.class), (Long) rs.getObject("school_id"),
                        ...
                        rs.getString("communication_category"), rs.getString("status"), rs.getString("dispatch_mode")))
    ...
    public void approve(Broadcast broadcast, List<Recipient> recipients, Long actorId) {
        ...
        jdbc.sql("UPDATE notification.notification_broadcasts SET status = 'APPROVED', approved_by = :actor, approved_at = now(), updated_at = now() WHERE id = :id AND school_id = :schoolId")
                .param("id", broadcast.id()).param("schoolId", broadcast.schoolId()).param("actor", actorId).update();
    }
    ...
    public Map<String, Object> outcomes(Broadcast broadcast) {
        List<Map<String, Object>> recipients = jdbc.sql("""
                SELECT student_id, channel, status, reason, attempts, next_attempt_at, provider, dry_run
                ...
                .query((rs, row) -> {
                    ...
                    result.put("provider", rs.getString("provider")); result.put("dryRun", rs.getBoolean("dry_run")); return result;
                }).list();
        ...
        result.put("delivered", 0); // Provider acceptance and dry runs are never recipient delivery receipts.
        return result;
    }
    ...
    public Optional<QueuedRecipient> claim(OffsetDateTime now) {
        ...
        try {
            claimed = jdbc.sql("""
                    ...
                    WHERE r.status IN ('QUEUED', 'FAILED') AND b.dispatch_mode = 'DRY_RUN'
                      AND (r.next_attempt_at IS NULL OR r.next_attempt_at <= :now)
                    ORDER BY r.created_at, r.id LIMIT 1 FOR UPDATE OF r SKIP LOCKED
                    """).param("now", now).query((rs, row) -> new QueuedRecipient(rs.getObject("id", UUID.class), rs.getObject("broadcast_id", UUID.class),

 ...
 * {@code SCHOOL_CORE_OUTBOX_PUBSUB_TOPIC} instead of {@code ..._TOPIC_ID}, so
 * {@code @ConditionalOnProperty("school-core.outbox.pubsub.topic-id")} never matched
...
public class OutboxPublisherStartupCheck {
    ...
    public OutboxPublisherStartupCheck(
            ...
            @Value("${school-core.outbox.require-real-publisher:true}") boolean requireRealPublisher) {
        ...
        if (publisher instanceof LoggingDomainEventPublisher) {
            throw new IllegalStateException(
                    "FATAL: 'prod' profile is active but the DomainEventPublisher is the no-op "
                            ...
                            + "Set the SCHOOL_CORE_OUTBOX_PUBSUB_TOPIC_ID env var so the real "

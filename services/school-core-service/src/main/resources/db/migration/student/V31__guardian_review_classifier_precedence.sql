-- Rebuild the review-only classifier around the governed evidence partition.
-- This migration changes labels only: case IDs, snapshot digests, source data,
-- guardian relationships, consent records, and recorded decisions are untouched.

ALTER VIEW student.guardian_data_review_queue_v1
    RENAME TO guardian_data_review_queue_v30;
...
corrected AS (
    SELECT
        queue.*,
        CASE
            -- A canonical guardian value with an empty legacy projection is a
            -- projection gap. V29 intentionally compared blank values as empty,
            -- but its classifier only checked SQL NULL.
            ...
            WHEN queue.field_name = 'name'
                ...
                THEN 'PROJECTION_MISSING'
            -- Casing-only name differences stay formatting cases even when the
            -- linked guardian also belongs to a large phone cluster.
            WHEN queue.field_name = 'name'
                ...
                THEN 'CASE_ONLY'
            -- A very large complete legacy-phone cluster is established
            -- placeholder contamination. A smaller repeated cluster remains a
            -- candidate so the two risk levels cannot be bulked together.
...
REVOKE ALL ON TABLE student.guardian_data_review_queue_v30 FROM PUBLIC;
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'app_rt') THEN
        GRANT SELECT ON student.guardian_data_review_queue_v30 TO app_rt;

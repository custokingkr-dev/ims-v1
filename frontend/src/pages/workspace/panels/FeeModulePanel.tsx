...
import { paiseToRupeeInput } from '../utils';
...
export function FeeModulePanel({ workspace, onRefresh, initialView = 'overview' }: FeeModulePanelProps) {
  ...
  const currency = (paise: number) => formatSchoolCurrency(Number(paise || 0) / 100, workspace?.school);
    ...
    >
      <div className="erp-module">
        <div className="erp-tabs" role="tablist" aria-label="Fee module">
          <button className={view === 'overview' ? 'active' : ''} onClick={() => setView('overview')}><CircleDollarSign size={16} /> Collections</button>
        ...
        {view === 'overview' && (
          <>
            <div className="erp-metrics">
              ...
              <div className="erp-metric"><span>Collected</span><strong>{currency(summary.collected)}</strong><small>posted payments</small></div>
              <div className="erp-metric"><span>Receivable</span><strong>{currency(Math.max(0, summary.target - summary.collected))}</strong><small>remaining balance</small></div>
                  ...
                  {!loading && filteredRows.map((row, index) => {
                    ...
                    return (
                      <tr key={String(valueOf(row, ['studentId', 'assignmentId'], index))}>
                        ...
                        <td>{currency(money(row, ['totalFee', 'totalAnnualFeePaise', 'netPayable']))}</td>
                  ...
                  {selectedBand ? (
                    <>
                      <header className="erp-section-head">
                        <div><span className="erp-eyebrow">Revision {selectedBand.revision}</span><h2>{selectedBand.name}</h2><p>Classes {selectedBand.classFrom}-{selectedBand.classTo} - {currency(selectedBand.annualTotal)} annually</p></div>
                      ...
                      <div className="erp-policy-strip">
                        ...
                        <span><strong>{selectedBand.lateFeeType === 'NONE' ? 'No late fee' : currency(selectedBand.lateFeeAmount)}</strong> {selectedBand.lateFeeType.toLowerCase()}</span>
                          ...
                          <table className="erp-table compact">
                            ...
                            <tbody>{selectedBand.items.map((item) => {
                              ...
                              return <tr key={item.id}>
                                ...
                                <td>{isEditing ? <input aria-label="Fee head amount" type="number" min="0" value={editingItem.amount} onChange={(event) => setEditingItem({ ...editingItem, amount: event.target.value })} /> : currency(item.amount)}</td>

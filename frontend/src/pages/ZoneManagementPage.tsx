  ...
  return (
    <div style={{ padding: '2rem', maxWidth: 960, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          <Link to="/schools" style={{ fontSize: 13, color: 'var(--ck-text-secondary)', textDecoration: 'none' }}>&larr; Schools</Link>
          <h1 style={{ margin: '0.25rem 0 0', fontSize: '1.4rem', fontWeight: 700 }}>Zone management</h1>
        </div>
        <button onClick={() => setShowZoneModal(true)}
          style={{ background: 'var(--ck-color-ink)', color: 'var(--ck-text-inverse)', border: 'none', borderRadius: 8, padding: '0.5rem 1rem', cursor: 'pointer', fontWeight: 600 }}>
          + New zone
        </button>
      </div>

      {error && <div style={{ background: 'var(--ck-color-danger-soft)', color: 'var(--ck-color-danger)', padding: '0.75rem 1rem', borderRadius: 8, marginBottom: '1rem' }}>{error}</div>}
      {notice && <div style={{ background: 'var(--ck-color-primary-soft)', color: 'var(--ck-color-primary)', padding: '0.75rem 1rem', borderRadius: 8, marginBottom: '1rem' }}>{notice}</div>}

      {loading ? <p>Loading zones…</p> : (
        <div className="ck-table-wrap">
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          <thead>
            <tr style={{ borderBottom: '2px solid var(--ck-border-subtle)' }}>
              {['Zone', 'Code', 'City', 'Schools', 'Admin', 'Actions'].map(h => (
                <th key={h} style={{ textAlign: 'left', padding: '0.5rem 0.75rem', fontWeight: 600, color: 'var(--ck-text-primary)' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            ...
            {zones.map(z => (
              <tr key={z.id} style={{ borderBottom: '1px solid var(--ck-border-subtle)' }}>
                ...
                <td style={{ padding: '0.75rem', color: z.adminEmail ? 'var(--ck-text-primary)' : 'var(--ck-text-muted)' }}>{z.adminEmail || 'Not set'}</td>
                <td style={{ padding: '0.75rem' }}>
                  <button onClick={() => { setSelectedZoneId(z.id); setShowAdminModal(true); }}
                    style={{ fontSize: 12, background: 'var(--ck-bg-app)', border: 'none', borderRadius: 6, padding: '0.3rem 0.6rem', cursor: 'pointer' }}>
                    Set admin
                  </button>
                </td>
              </tr>
      ...
      {showZoneModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <form onSubmit={createZone} style={{ background: 'var(--ck-bg-surface)', borderRadius: 12, padding: '2rem', width: 420, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {(['name', 'code', 'city', 'state', 'description'] as const).map(f => (
              <input key={f} required={f === 'name' || f === 'code'} placeholder={f.charAt(0).toUpperCase() + f.slice(1)}
                ...
                style={{ padding: '0.5rem 0.75rem', border: '1px solid var(--ck-border-subtle)', borderRadius: 8, fontSize: 14 }} />
            ))}
            <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
              <button type="button" onClick={() => setShowZoneModal(false)}
                style={{ padding: '0.5rem 1rem', border: '1px solid var(--ck-border-subtle)', borderRadius: 8, background: 'var(--ck-bg-surface)', cursor: 'pointer' }}>Cancel</button>
              <button type="submit" disabled={saving}
                style={{ padding: '0.5rem 1rem', border: 'none', borderRadius: 8, background: 'var(--ck-color-ink)', color: 'var(--ck-text-inverse)', cursor: 'pointer', fontWeight: 600 }}>
      ...
      {showAdminModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <form onSubmit={createAdmin} style={{ background: 'var(--ck-bg-surface)', borderRadius: 12, padding: '2rem', width: 420, display: 'flex', flexDirection: 'column', gap: '1rem' }}>

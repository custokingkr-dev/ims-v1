...
export function ProductFormBuilder({ categoryCode = 'NOTEBOOKS', definition: suppliedDefinition, schoolId, initialOrder, preview = false, onSaved }: Props) {
  ...
  } : { orderSelections: initialSelections(definition.groups, 'ORDER'), lines: [{ selections: initialSelections(definition.groups, 'LINE'), bookCount: 0, pageCount: 196 }] });
  ...
  return <div className="ck-product-form">
    ...
    <fieldset className="ck-product-fieldset" disabled={saving || isPlaced}>
      <div className="ck-product-fields">
        {groups.filter((group) => group.scope === 'ORDER').map((group) => <label className="field" key={group.code}><span>{group.label}</span>{optionSelect(group, input.orderSelections[group.code], (value) => setInput((current) => ({ ...current, orderSelections: { ...current.orderSelections, [group.code]: value } })), group.label, input.orderSelections)}</label>)}
      ...
      <div className="ck-product-lines">
        {input.lines.map((line, index) => <div className="ck-product-line" key={index}>
          <div className="ck-product-line-fields">{groups.filter((group) => group.scope === 'LINE').map((group) => <label className="field" key={group.code}><span>{group.label}</span>{optionSelect(group, line.selections[group.code], (value) => setInput((current) => ({ ...current, lines: current.lines.map((item, i) => i === index ? { ...item, selections: { ...item.selections, [group.code]: value } } : item) })), `${group.label} for line ${index + 1}`, { ...line.selections, ...input.orderSelections })}</label>)}
            <label className="field"><span>Books</span><input aria-label={`Books for line ${index + 1}`} type="number" min="1" step="1" value={line.bookCount || ''} onChange={(e) => setInput((current) => ({ ...current, lines: current.lines.map((item, i) => i === index ? { ...item, bookCount: Number(e.target.value) } : item) }))} /></label>
            <label className="field"><span>Printed pages</span><input aria-label={`Printed pages for line ${index + 1}`} type="number" min="1" step="1" value={line.pageCount || ''} onBlur={() => setBlurred((current) => ({ ...current, [index]: true }))} onChange={(e) => { setBlurred((current) => ({ ...current, [index]: false })); setInput((current) => ({ ...current, lines: current.lines.map((item, i) => i === index ? { ...item, pageCount: Number(e.target.value) } : item) })); }} />
      ...
      <button type="button" className="ck-btn ck-btn-ghost" onClick={() => setInput((current) => ({ ...current, lines: [...current.lines, { selections: initialSelections(groups, 'LINE'), bookCount: 0, pageCount: 196 }] }))}><Plus size={16} aria-hidden="true" />Add line</button>

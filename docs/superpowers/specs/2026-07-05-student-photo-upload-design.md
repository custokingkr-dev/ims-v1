...
- Frontend already sends `multipart/form-data` (field `file`, <=5 MB, JPG/PNG/WEBP) and renders `student.photoUrl` directly as `<img src>`.
...
**Cost + latency lever — resize on upload.** Server-side downscale + center-crop to a **512×512 JPEG (~q0.82, ~40–60 KB)** with Thumbnailator, a large reduction from a 5 MB upload cap. Objects are written with `Cache-Control: public, max-age=1y, immutable` (content-addressed → never change) so browsers cache them. At 50 KB × 60k photos ≈ 3 GB → **~$0.06/mo** GCS Standard; egress stays low from small, cached images.
...
2. **`StudentReadController.attachPhoto`** — accept `@RequestParam("file") MultipartFile`, validate content-type (jpeg/png/webp) and size (<=5 MB), call `StudentPhotoStorage.upload`, persist the returned object key, return the (signed) student detail.
...
4. **Config** (`application.yml`, env-driven): `student.photo.bucket`, `.signed-url-ttl-minutes` (60), `.max-bytes` (5 MiB), `.dimension` (512); Spring `servlet.multipart.max-file-size=5MB`.

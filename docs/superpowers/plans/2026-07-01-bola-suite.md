...
- **Oracle (per probe type):**
  ...
  - **Inconclusive status = failure, not silent pass** (all probe types): a status of `0` (connection error/timeout) or `>= 500` means the probe did NOT exercise the isolation boundary — flag it as a failure (`Add-Failure "… probe inconclusive — unexpected status …"`), never let it fall into a "pass" arm. A gate that exits 0 because a service is down is worse than useless. (Established in Task 2's detail oracle; the list oracle MUST do the same.)
- **PowerShell 5.1 interpolation:** put a `?` or `&` after a variable in a double-quoted URL with `${var}` braces — `"…/students/${StudentB}?schoolId=${SchoolB}"`, NOT `"…/students/$StudentB?schoolId=$SchoolB"`. PS 5.1 treats `?` as a valid identifier char, so the unbraced form parses `$StudentB?schoolId` as one (empty) variable and silently hits the wrong path.

...
- **Status (2026-07-01): done** — `phase1-bola` / `docs/superpowers/plans/2026-07-01-bola-suite.md`. `scripts/audit-tenant-isolation.ps1` implements a two-school seeded fixture and a positive baseline. **Honest coverage:** the gate has real isolation **teeth** (seeded school-B marker rows that must never appear in a school-A response) on **students, catalog orders, firefighting requests, and workflow instances** — across detail-by-id and marker-list vectors. The **aggregate endpoints** (attendance daily-summary, fee report, catalog annual-plan, firefighting stats, workspace dashboard) are **advisory equivalence probes** only (deny-or-same-as-own-scope), with no differential per-school seed data to give them teeth. Global endpoints (e.g. `classes`, no `school_id`) are excluded. Gate is wired into `verify-microservice-migration.ps1` via `-RunBolaAudit` and runs in `whole-application-validation.yml` CI (gateway in enforce mode via `docker-compose.bola.yml`). **No leaks found on the teeth-backed probes.** The **Phase 1 gate is now met**: BOLA teeth-backed probes green (Task 1.5) AND RLS blocks cross-tenant queries on all NOT NULL `school_id` tables (Task 1.3 + 1.4 extension, branch `phase1-rls-extension`).
- **FOLLOW-UP:** seed differential per-school data for the aggregate endpoints (and negative admin-B probes) to give those probes teeth.
...
- **Status (2026-07-01): done** — `phase1-refresh-rotation` / `docs/superpowers/plans/2026-07-01-refresh-token-rotation.md`. Refresh tokens now rotate per login-family: each `auth_sessions` row carries `family_id`/`status`(`ACTIVE`/`ROTATED`/`REVOKED`)/`rotated_at` (Flyway V2; existing rows backfilled to single-token `ACTIVE` families). On a valid refresh the presented token is retired (`ROTATED`) and a new `ACTIVE` row is inserted in the same family. Replaying a retired or revoked token (`status != ACTIVE`) triggers whole-family revocation (`UPDATE … WHERE family_id = ? → REVOKED`) + an `REFRESH_TOKEN_REUSE_DETECTED` row written to `identity.rbac_audit_log` — committed even on the resulting 401 (`@Transactional(noRollbackFor=ResponseStatusException.class)`). Logout revokes the whole family. Blast radius is family-only; access TTL unchanged (~15 min / 900 s). Tests: `AuthSessionMigrationTest` (2 — Testcontainers, backfill proof), `IdentityAuthServiceRotationTest` (7 — Mockito, covers rotate/reuse/family-scoped-blast-radius/logout), `IdentityAuthServiceReuseCommitIntegrationTest` (1 — Testcontainers, proves audit row survives 401 rollback).

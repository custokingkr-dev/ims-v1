...
## Live EMAIL follow-up deployment

The dedicated EMAIL implementation in [PR 294](https://github.com/custokingkr-dev/ims-v1/pull/294) is deployed to dev at `a07c2fa73a4bc8731b2d41015f31f65624bfdb8c`. [Release 36261225626](https://github.com/custokingkr-dev/ims-v1/actions/runs/36261225626) passed tests, image security checks, rollout, serving-digest/traffic verification, gateway smoke, signing and dev approval tagging. CI passed 345 platform tests, 88 gateway tests, 397 frontend tests and 109 browser checks. The preceding configuration reconciliation passed separately. Backup `1790446335149` completed at 18:13:36 UTC before application deployment began.

Post-deployment checks on `custoking-platform-service-dev-muipmx2l` confirmed school 1 remains DRY_RUN with live sending disabled. Missing or forged callback credentials returned 401; two authenticated reports for a synthetic unknown correlation returned 202. Read-only SQL execution `ims-q-dev-xj2hr` confirmed V12 successful, zero live submissions/reports in school 1, both ledger tables under RLS, and the restricted runtime grants: SELECT/INSERT allowed, table-wide UPDATE/DELETE/TRUNCATE denied, immutable request hash denied UPDATE, outcome column allowed UPDATE. Both retained broadcasts are still DRY_RUN_COMPLETE with DRY_RUN approval and dispatch modes. Natural Scheduler calls returned 200 every minute from 18:26 through 18:30 UTC on the new revision. No provider submission occurred.

The first verification login returned server HTTP 200 after 33.62 seconds, beyond the client's 30-second deadline; a follow-up took 0.48 seconds. [PR 296](https://github.com/custokingkr-dev/ims-v1/pull/296) gives only the login POST a 60-second deadline without retries or refresh fallback. Its exact-head CI passed 401 frontend tests and 109 browser checks; CodeQL passed. [Dev release 36262889154](https://github.com/custokingkr-dev/ims-v1/actions/runs/36262889154) succeeded at source `1da0dc1d99f839bdd6b24aeb6565bb85d7728f63`: frontend `custoking-frontend-dev-00050-snx` is ready at 100% traffic and returned HTTP 200; gateway health is UP, and signing/dev approval tagging completed. Evidence is retained in `release-evidence-dev-1da0dc1d/`. A school participant's actual login experience remains part of human acceptance.

Additional local evidence includes `release-evidence-dev-a07c2fa7/`, `live-dev-backup.json`, `live-release-dev-verification.json`, `live-postdeploy-database-proof.json`, `live-postdeploy-scheduler-proof.json`, `live-login-request-evidence.json` and the preserved initial failed verification journal. These checks establish deployed controls, not real recipient delivery or human school acceptance.
...
- **Live messaging:** the subsequent dedicated EMAIL implementation uses a durable single-submission reservation and authenticated reports. Actual provider access, sender/template verification and recipient delivery remain open; the current dev credential returned HTTP 401 on a read-only template lookup. See [the current live messaging evidence](live-messaging-readiness-2026-09-26.md). Keep broadcast processing in dry-run until those checks pass.
...
Production promotion was subsequently requested after live and school acceptance; see [production readiness](production-promotion-readiness-2026-09-26.md). It has not been performed. SSO remains deferred.

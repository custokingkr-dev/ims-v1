...
Where the operator has public application access but cannot invoke the private peer, explicitly select `-PolicyEvidenceSource PublicManifest`. This mode reads no policy secret and obtains no private identity token. It verifies the exact synthetic guardian and current consent through the student workspace/guardian APIs, requires a fresh preview with exactly one eligible recipient, then verifies the stored approved manifest binds that recipient to the owned student and leaves every other recipient suppressed with zero attempts before queueing. Aggregate preview counts alone never authorize queueing. After withdrawal, a fresh preview of an already journaled owned broadcast must have zero eligible recipients; actual queue outcomes separately prove the worker's fresh consent check. If a failure leaves no owned broadcast available for this final policy check, cleanup remains explicitly unresolved.

The helper creates only the synthetic guardian and explicitly labelled consent, uses an `acceptance.invalid` email, and previews at most 200 recipients. It requires exactly one eligible destination with no duplicates, binds it to the owned student/guardian/grant through the selected policy evidence path, and checks the stored approved list before queueing. Existing denied recipients must remain suppressed with zero attempts. It checks one dry-run attempt, queue replay without another attempt, and withdrawal **before** a second queue with a suppressed outcome. Both queues require paired natural Scheduler and Cloud Run 2xx evidence after acceptance, on the ready revision; the helper never manually calls the drain. Zero delivered means no recipient delivery receipt, not verified inbox delivery.

The business limits are 55 API requests, 30 cloud reads, and 12 minutes. Cleanup has a separate ten-request, three-minute reserve, including one reserved logout request. It reconciles only the exact run-owned student and synthetic guardian, withdraws any remaining test grant with a stable key, and verifies denial through the selected policy evidence path. An uncertain cleanup remains explicitly incomplete. Journals use exclusive creation at `artifacts/product-dev-release-2026-09-26/<runId>-broadcast-journal.json` and contain synthetic payloads/IDs and safe outcomes, never credentials or ordinary guardian destinations. An existing journal blocks restarting. Preserve the journal after any timeout or uncertain write; no new-key retry is attempted.
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\tests\dev-broadcast-public-policy-test.ps1
...
Scheduler evidence uses a structured Logging API request scoped to `projects/custoking-dev`. This preserves quoted timestamp/URL filters on Windows and avoids a native command-line parsing failure. Successful log matches must still identify the expected job, URL and serving revision.

### Failed-attempt reconciliation

Preserve failed journals. A preflight-only attempt may be archived only after confirming it made no fixture writes and any login was logged out; keep the same run-owned student and markers. A failure after creation requires authoritative readback of the saved IDs, never a fresh blind run. The 26 September HTTP 415 queue attempt was reconciled from its existing approved manifest with zero attempts, the exact withdrawn synthetic consent, and a separately journaled recovery. The queue helper now sends an explicit empty JSON object. A new synthetic grant after a confirmed withdrawal uses its own stable event key; a later cleanup decision also needs a new stable key. Reusing the first withdrawal's key cannot represent a later consent decision.

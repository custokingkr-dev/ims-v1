...
## 8. The twenty defects, and the seven that only production could produce

All are already fixed in the scripts or the repository. They are listed so that a failure at two in the
morning is recognised rather than debugged. Rows 1–13 came from the dev rehearsal; rows 14–18 came from
building and then actually *releasing to* `custoking-prod` and **could not have been caught on dev at any level of diligence**.
...
| 19 | Cloud Deploy release IDs are `rel-{env}-{sha12}-{run_attempt}`, and a **fresh workflow_dispatch always has `run_attempt` = 1**. Retrying a failed production release by dispatching again therefore collides with `ALREADY_EXISTS` forever, because Cloud Deploy releases are immutable. The retry path is `gh run rerun --failed`, which increments the attempt. | every retry after a failed prod release. Invisible on dev, which does not use Cloud Deploy |
| 20 | Cloud Deploy targets are **rendered artifacts, not live lookups**. Correcting a GitHub variable does nothing to an already-applied target; it must be re-rendered and re-applied. | any variable fix between reconcile and release -- the release silently keeps using the value baked in at reconcile time |
| 17 | **Production only.** A stale **repository-level** `PROD_DB_HOST` still pointed at the source project's database IP and beat the prod environment's `DB_HOST`, because the workflow reads `vars.PROD_DB_HOST \|\| vars.DB_HOST`. Setting a variable at environment scope does not shadow a repository variable of a *different name* that takes precedence. | first release after cutover. It failed closed only because that address is unallocated in the destination VPC -- the two projects use overlapping RFC1918 ranges, so a listener on it would have produced a **healthy revision against the wrong database** |
| 18 | **Production only.** Cloud Deploy renders the entire Cloud Run spec from `deploy/cloudrun/*.yaml`, so `STUDENT_PHOTO_IMPORT_CREDENTIAL_MODE`, applied by hand with `gcloud run services update`, was silently dropped by the release. | a *successful* release, which would have reverted Drive to the OAuth client owned by the deleted source project **with every health check green** -- nothing calls Drive until an operator runs a photo import, so it would have surfaced days later with no way back |
| 15 | **ROOT-CAUSED 2026-08-19.** `gcloud storage rsync --exclude` **silently drops the first alternative of a top-level regex alternation**. `^temporary/.*\|^students/.*\|^student-imports/.*` copied all 122 `temporary/` objects while correctly excluding the other two prefixes. The trailing `.*` was never the problem — the same pattern used alone works, and prefixing a throwaway `DUMMY\|` alternative also makes the real ones work, proving position rather than syntax is at fault. Fixed by grouping: `^(temporary\|students\|student-imports)/`. Measured against an empty scratch bucket, because a destination that already mirrors the source reports nothing to transfer and hides the bug. | photo copy; any `--exclude` using alternation anywhere |

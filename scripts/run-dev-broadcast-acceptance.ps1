...
function Remaining-Seconds { return [int][Math]::Floor(($deadline-[datetime]::UtcNow).TotalSeconds) }
...
function Api([string]$Method,[string]$Path,$Body=$null) {
  ...
  if ($script:count -ge $(if($logout){55}else{54}) -or (Remaining-Seconds) -le $(if($logout){0}else{12})) {throw 'Bounded request or duration limit reached'}
  $script:count++
...
function Read-OwnedPolicy([string]$Id,[bool]$RequireAllowed=$true) {
  Assert ($script:count -lt 54 -and (Remaining-Seconds) -gt 12) 'Bounded request or duration limit reached';$script:count++
...
try {
  ...
  if($authenticated){try {Api POST '/auth/logout'|Out-Null;$state.logoutConfirmed=$true}catch{$state.logoutConfirmed=$false;$state.completed=$false;$state.logoutFailure='Logout was not confirmed; server session cleanup remains required.'}}
...
[ordered]@{completed=$state.completed;runId=$RunId;dryRun=$state.dryRun;replay=$state.replay;revoked=$state.revoked;logoutConfirmed=$state.logoutConfirmed;journal=$journalPath}|ConvertTo-Json -Depth 8

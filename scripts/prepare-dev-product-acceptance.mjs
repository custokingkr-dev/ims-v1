...
export function buildAcceptancePlan(config) {
  ...
  return {
    ...
    role: 'A school-scoped ADMIN actor with student:create/read, attendance:manage/read, fee:assign/collect/read, firefighting:create/update/read, plan:manage/read and required enabled modules.',
    ...
    preflight: [
      request('actor', 'GET', '/api/v1/auth/me', undefined, ['user school/branch equals schoolId', 'school-scoped role and necessary permissions; not a superadmin bypass']),
